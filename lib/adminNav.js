...
export const ADMIN_NAV = [
    ...
    /* ⚑ "Packages" used to sit here, between Services and Blogs. The public
       /packages route no longer reads from the API — its prices are static, in
       str-frontend/lib/pricingData.js — so there is nothing behind that screen
       to edit and the entry would have opened a 404. Changing a price is a code
       change and a deploy now; that is the trade that came with dropping the
       collections. Only the <head> for the route is still editable, at Page
       meta → Packages. */
    ...
    /* Sits beside Inquiries because both are "someone wrote in", but it is its
       own collection and its own screen: a graphics order carries a file
       manifest and a deadline, and the one thing the studio has to see fast is
       an order whose attachments never left (mailStatus: failed). */
    {
        href: "/admin/graphics-quotes",
        label: "Graphics orders",
        icon: GraphicsQuotesIcon,
        group: "Inbox",
    },
