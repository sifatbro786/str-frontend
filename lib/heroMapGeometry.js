...
/* ⚑ createRequire, not a static JSON import, and that is deliberate.
   This module is BUILD-ONLY — scripts/build-hero-map.mjs is its only importer,
   and its output is lib/heroMapPaths.json, which is what the app ships. The
   node: specifier enforces that boundary mechanically: if anyone ever imports
   this file from a component again, the bundler fails on `node:module` instead
   of quietly pulling 105 KB of topology and the whole of d3-geo back into the
   client bundle, which is the exact regression this split exists to prevent.

   (The practical reason is also real: plain Node ESM needs an import attribute
   for JSON, which webpack and Turbopack disagree about. createRequire works
...
/**
 * lib/heroMapGeometry.js — the geometry behind the hero's service-network map.
 *
 * ── ⚑ THIS FILE DOES NOT SHIP. ───────────────────────────────────────────
 * It runs once, at build time, from scripts/build-hero-map.mjs, which writes
 * its output to lib/heroMapPaths.json. Components import lib/heroMap.js, which
 * re-exports that JSON and nothing else.
 *
 * It used to be lib/heroMap.js, imported directly by GeoWorldMap.jsx — which
 * is "use client", so d3-geo, topojson-client and world-atlas's 105 KB
 * countries-110m.json were all being shipped to every visitor, on the homepage,
 * to recompute path strings that are byte-identical on every build. Everything
 * below is a pure function of constants, so none of it needs to be in a browser.
 *
 * Adding a market or changing the projection means re-running the build script;
 * `npm run prebuild` does it, and `npm run build` runs prebuild automatically.
 *
...
/* ── Frame ───────────────────────────────────────────────────────────────
   The frame has to contain the whole disc, and it did not. With CY at 330 and
   RADIUS at 290 the disc ran from y=40 to y=620 inside a 560-tall viewBox, so
   an SVG root — which clips to its viewBox — sliced 60px off the bottom. The
   ocean fill, the graticule and the disc's own outline stroke all ended on a
   hard horizontal line, which is the "unfinished circle" the design review
   picked up: a cropped circle reads as a rendering failure, never as a crop.

   The frame is now sized from the disc rather than the other way round:

       top gap     CY - RADIUS          12
       disc                            580
       bottom gap  HEIGHT - CY - RADIUS  28   (the legend row sits here)

   ⚑ These three constants are coupled. If RADIUS or CY changes, check
   CY + RADIUS <= HEIGHT - 24 or the crop comes straight back. */
...
const CX = WIDTH / 2;
/* Slightly above the geometric middle (310), which leaves the 28px the legend
   row needs under the disc and nothing more.

   This used to sit at 330, pushing the disc's southern rim out of the frame on
   the argument that every market except Australia and Singapore is north or
   west of Dhaka, so the bottom of the disc is empty Southern Ocean and not
   worth the pixels. The argument is sound; overflowing the viewBox is not the
   way to act on it, because the result is a hard-edged half-circle rather than
   a bleed. If the southern rim ever needs to go again, it goes by lowering
   CLIP — which removes the empty degrees from the projection itself and leaves
   a complete, smaller disc — not by drawing outside the frame. */
const CY = 302;
const RADIUS = 290;
