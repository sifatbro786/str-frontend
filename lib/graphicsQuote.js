...
/**
 * Passes that are orderable but have no showcase block of their own.
 *
 * The checkbox row on the order desk is built from GRAPHICS_SERVICES so that a
 * pass can never be orderable under a name the page does not publish. These
 * two are the exception: the studio takes the work, there is simply no before
 * and after pair on /graphics for it yet. They live here rather than in
 * lib/graphics.js because that array also builds the showcase and the rate
 * table, and a row in it with no imagery and no per-image rate would render an
 * empty compare frame and a blank price.
 *
 * ⚑ THE COUNT IS PART OF THE CONTRACT. servicesRequired is capped server side
 * in str-backend/src/validators/graphicsQuote.validator.js, and the cap has to
 * cover every box on the page — eight published passes plus these two is ten.
 * Add a third here without raising it and a client who ticks everything gets a
 * 400 on a form they filled in correctly.
 */
export const EXTRA_PASSES = ["Multi Clipping Path", "Vector Illustration"];
