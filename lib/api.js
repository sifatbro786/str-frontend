...
/* ── Packages ─────────────────────────────────────────────────────────────
   The bilingual pricing route. ONE call returns the whole page — the copy
   block, every track, and every tier nested under its track, with both locales
   on each — because that is exactly what /packages renders and there is no
   second consumer to generalise for. The shaping is done server-side; see
   str-backend/src/controllers/package.controller.js for why.

   Not wrapped: an outage here is a 500 on a cold route, which is the honest
   answer, and ISR keeps serving the last good render on a failed revalidation.
   Returning an empty catalogue instead would render a pricing page with no
   prices on it and then cache that for the full window.

   ⚑ Adding a field means the model, the validator and the controller's
   projection move together — the projection is a `.select()`, so a field added
   to the schema and not to it simply never arrives, with nothing reporting it.
   ──────────────────────────────────────────────────────────────────────── */

export const getPackages = async () =>
    (await apiFetch("/packages", { tags: ["packages"] })).data;
