...
/* ── Packages ─────────────────────────────────────────────────────────────
   Nothing here any more, on purpose. /packages used to be the bilingual BDT
   catalogue and this file carried getPackages(), a single call against
   /api/v1/packages that returned the copy block, every track and every tier in
   one shaped response. That whole stack was removed — the three models, the
   controller, the router, the validator, the seed, the admin screen — and the
   route now renders lib/pricingData.js, which is static.

   The "packages" cache tag went with it: nothing tags a fetch with it, so
   nothing can revalidate it. It is gone from the TAGS set in
   app/api/revalidate/route.js and from the proxy allow-list in
   app/api/admin/[...path]/route.js, and those three have to stay in step.
   ──────────────────────────────────────────────────────────────────────── */
