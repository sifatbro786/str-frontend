    ...
    return (
        <g data-land="" pointerEvents="none" shapeRendering="optimizeSpeed">
            {LAND_PATHS.map((country, i) => {
                const tint = country.discipline ? DISCIPLINES[country.discipline].color : null;
        ...
        <div ref={root} className={className}>
            <svg
                ...
                aria-label={`Service network centred on Dhaka. ${markets
                    .map((m) => `${m.name}, ${m.distance}, ${DISCIPLINES[m.discipline].label}`)
                    .join(". ")}`}
            >
                ...
                {markets
                    ...
                    .map((m) => (
                        <foreignObject
                            ...
                            data-card={m.code}
                            suppressHydrationWarning
                            x={Math.min(Math.max(m.label.x - CARD_W / 2, 4), WIDTH - CARD_W - 4)}
                            y={Math.min(Math.max(m.label.y - CARD_H / 2, 4), HEIGHT - CARD_H - 4)}
                            width={CARD_W}
                            height={CARD_H}
