    ...
    return (
        <div className="overflow-x-auto border border-(--line)">
            <table
                ...
                className={cn(
                    "w-full min-w-3xl border-collapse text-left",
                    busy && "opacity-60",
                )}
            >
                <thead>
                    <tr className="border-b border-(--line)">
                        {columns.map((c) => (
                            <th
                                ...
                                className={cn(
                                    "label-mono px-4 py-3 font-medium text-(--text-mute)",
                                    c.className,
                                )}
                            >
                                {c.header}
                        ...
                        {actions && (
                            <th
                                scope="col"
                                className="label-mono px-4 py-3 text-right font-medium text-(--text-mute)"
                            >
                                Actions
                            </th>
                        )}
                ...
                <tbody>
                    {rows.length === 0 ? (
                        ...
                        rows.map((row) => (
                            <tr
                                key={rowKey(row)}
                                className="border-b border-(--line-soft) transition-colors last:border-0 hover:bg-(--raised-2)"
                            >
                                {columns.map((c) => (
                                    <td
                                        key={c.key}
                                        className={cn(
                                            "px-4 py-3.5 align-middle text-[0.9375rem] text-(--text-dim)",
                                            c.className,
                                        )}
                                    >
                                ...
                                {actions && (
                                    <td className="px-4 py-3.5 align-middle">{actions(row)}</td>
                                )}
                            </tr>
                        ))
                    )}
