...
export default function Topbar({ user, onMenu }) {
    ...
    const current =
        ADMIN_NAV.find((i) => (i.exact ? pathname === i.href : pathname.startsWith(i.href)))
            ?.label ?? "Admin";
        ...
        <header className="sticky top-0 z-30 flex h-16 items-center gap-4 border-b border-(--line) bg-(--canvas) px-5 sm:px-8 lg:px-10">
            ...
            <div className="ml-auto flex items-center gap-4">
                ...
                <div className="hidden text-right sm:block">
                    <p className="text-[0.8125rem] leading-tight font-medium text-(--text)">
                        {user?.name}
                    </p>
                    <p className="label-mono leading-tight text-(--text-mute)">
                        {user?.role?.replace("_", " ")}
                    </p>
                </div>
                <button
